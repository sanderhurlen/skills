...
- **GitHub**: URL contains `github.com` → file RFC as a GitHub issue with `gh issue create`. Extract `owner/repo`.
- **Azure DevOps**: URL contains `dev.azure.com` → file RFC as a work item with `az boards work-item create`. Extract `ORG_URL` and `PROJECT`.
